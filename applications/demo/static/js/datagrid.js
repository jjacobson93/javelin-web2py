...
(function($) {
	...
	var Datagrid = function (element, options) {
		...
		this.$pageinput = this.$element.find('.grid-pager select'); //.grid-pager input
		this.$pagedropdown = this.$element.find('.grid-pager select'); //.grid-pager .dropdown-menu
		...
		if (this.$pagesize.hasClass('select')) {
			this.options.dataOptions.pageSize = parseInt(this.$pagesize.select('selectedItem').value, 10); //parseInt(this.$pagesize.find('li[data-selected="true"]').attr('data-value'));
		} else {
	...
	Datagrid.prototype = {
		...
		updateColumns: function ($target, direction) {
			var className = (direction === 'asc') ? 'icon-caret-up' : 'icon-caret-down';
			this.$colheader.find('i').remove();
			this.$colheader.find('th').removeClass('sorted');
			$('<i>').addClass(className + " pull-right").appendTo($target);
		...
		renderData: function () {
			...
			this.options.dataSource.data(this.options.dataOptions, function (data) {
				...
				// self.$pageinput.select2();
				// self.$pageinput.select2('val', data.page); //val(data.page);
				// self.page = data.page;
