...
$(function() {
	...
	crewsTable = $('#crews-table').jTable({
		...
		pageSize: 100,
		ajax: {
			source: "/demo/orientation/call/json/crews",
			error: function() {
	...
	crewRecTable = $('#crew-records-table').jTable({
		...
		pageSize: 25,
		ajax: {
			source: "/demo/orientation/call/json/crew_records",
			data: {
				id: 0
			},
			error: function() {
		...
		createdRow: function(row, data) {
			...
			$(row).find('button[id^="crew-move-person"]').popover({
				...
				$('input#change-crew-select').select2({
					...
					ajax: {
						type: 'POST',
						url: '/demo/orientation/call/json/crews',
						dataType: 'json',
						quietMillis: 100,
						data: function(term) {
							return { id: term }
	...
	$('#add-crew-btn').on('click', function() {
		$('#add-person-select-modal').select2({
			...
			ajax: {
				type: 'POST',
				url: '/demo/orientation/call/json/people_not_in_crew',
				dataType: 'json',
				quietMillis: 100,
...
function makeNametags(type, event_name, present, event_id) {
	...
	$.ajax({
		type: 'POST',
		url: '/demo/orientation/call/json/make_labels',
		data: {
...
function makeAttSheets() {
	...
	$.ajax({
		type: 'POST',
		url: '/demo/orientation/call/json/attendance_sheet',
		data: {
			'kind': 'crew_freshmen',
		},
		dataType: 'json',
...
function addCrew(room, wefsk, people) {
	$.ajax({
		type: "POST",
		url: "/demo/orientation/call/json/add_crew",
		data: {
			'room' : room,
			'wefsk' : wefsk,
...
function addPeopleToCrew(id, people) {
	$.ajax({
		type: "POST",
		url: "/demo/orientation/call/json/add_people_to_crew",
		data: {
			'id': id,
			'people': JSON.stringify(people)
		},
...
function removeFromCrew(person_id) {
	$.ajax({
		type: "POST",
		url: "/demo/orientation/call/json/remove_crew",
		data: {
			'person_id': person_id
		},
		success: function() {
...
function changeToCrew(crew_id, person_id) {
	$.ajax({
		type: "POST",
		url: "/demo/orientation/call/json/move_to_crew",
		data: {
			'person_id': person_id,
			'id': crew_id
		},
...
function updateRoom(room, wefsk) {
	...
	$.ajax({
		type: "POST",
		url: '/demo/orientation/call/json/update_room',
		data: {
			'id': id,
			'room' : room,
			'wefsk': wefsk
