app.directive('ngJtable', ['$interpolate', '$http', '$sce', function($interpolate, $http, $sce) {
	...
	return {
		...
		'		</thead>' +
		'		<tbody>' +
		'			<tr ng-show="isLoading" style="text-align:center;font-weight:bold;"><td style="border:none" colspan="' + START + 'columns.length' + END + '"><i class="fa fa-circle-o-notch fa-spin"></i> Loading Data</td></tr>' +
		'			<tr ng-show="rows.length == 0 && !isLoading" style="text-align:center;font-weight:bold;"><td style="border:none" colspan="' + START + 'columns.length' + END + '">No Results</td></tr>' +
		'			<tr ng-repeat="row in rows" ng-click="rowClick(row)"><td ng-repeat="col in columns" ng-bind-html="\'<span>\' + (row[col.key] || trustAsHtml(col.template)) + \'</span>\'"></td></tr>' +
		'		</tbody>' +
		'	</table>' +
		'</div>' +
		'<div class="row">' +
		'	<div class="col-sm-6">' +
		'		<span>Showing ' + START + ' startIndex + 1 ' + END +' to ' + START + ' endIndex ' + END + ' of ' + START + ' numberOfEntries ' + END + ' entries &nbsp;&nbsp;&nbsp;</span>' + 
		'		<select ui-select2 ng-model="pageSizeSelect.selected" ng-change="pageSizeSelect.change()">' +
